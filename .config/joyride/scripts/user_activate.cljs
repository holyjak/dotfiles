...
(defn on-opened-doc-hide-repl-linenums [doc] 
  (when (str/ends-with? (.-fileName doc) "/.calva/output-window/output.calva-repl")
    (let [repl-editor
          (->> vscode/window.visibleTextEditors
               ;(filter #(-> % .-document .-fileName (str/ends-with? "/.calva/output-window/output.calva-repl")))
               (filter #(= (.-document %) doc))
               first)]
      (str "on-opened-doc-hide-repl-linenums editor = " repl-editor)
      (set! (.-lineNumbers (.-options repl-editor)) 0))))
...
(defn- my-main []
  ...
  ;(vscode/workspace.onDidOpenTextDocument on-opened-doc-hide-repl-linenums)
    ...
    (fn [doc]
      (when (str/ends-with? (.-fileName doc) "/.calva/output-window/output.calva-repl")
        (let [repl-editor
              (->> vscode/window.visibleTextEditors
               ;(filter #(-> % .-document .-fileName (str/ends-with? "/.calva/output-window/output.calva-repl")))
                   (filter #(= (-> % .-document .-fileName) (.-fileName doc)))
                   first)
              names (map #(-> % .-document .-fileName) vscode/window.visibleTextEditors)]
          (.appendLine (joyride/output-channel) (pr-str "on-opened-doc-hide-repl-linenums editor = " repl-editor names))
          (set! (.-lineNumbers (.-options repl-editor)) 0)))))
  )
